...
The null hypothesis for Question 1 is that concentrations of microcystin and chlorophyll are not significantly different and have no significant relationship between the various months and years in which the GLERL sampled them. Our alternative hypothesis is that there is a substantial difference between microcystin concentrations each month and each year. 

The initial plan was to conduct a time series analysis of the data to answer question 2, how microcystin concentrations change over time. This time series analysis would visualize microcystin and chlorophyll concentrations over month and year periods. However, after attempts to conduct a time series analysis and a closer look at the data, the gaps between data collection and other outliers were too extensive, making the visualization of time series data inaccurate and confusing. 

To assess whether differences in chlorophyll a and temperature between months and years were statistically significant, we conducted two-way ANOVA tests. ANOVA, a different statistical analysis used to determine the variability between multiple factors, was implemented to evaluate the statistical significance of differing microcystin and chlorophyll concentrations over the years. After assessing the ANOVA for microcystin and chlorophyll concentrations over the various studied months, years, and temperatures, we applied a Tukey HSD test to the results if the ANOVA P value was statistically significant. 
...
#class(Erie.Anova.analysis$Date)
# colnames(Erie_Data_2012_2018)
Erie.Anova.analysis <-
  Erie.Anova.analysis %>% 
  mutate(Month = month(Date))%>%
...
#because the P value of the anova test is less than .05 we ran a Tukey HSD test to determine which values differences were statistically significant. The results indicate that the microcystin concentrations from 2014 are statistically significantly different from every other year and that the difference between 2016 and 2018 is significant.
...
The ANOVA results differed from the alternative hypothesis, saying that the differences in microcystin and chlorophyll concentrations were not statistically significant except on a few occasions. The only model that demonstrated statistical significance was the ANOVA, which evaluated microcystin concentrations by year. That model found that 2014 microcystin concentrations were significantly different in 2014 and then between 2016 and 2018. These results primarily support our null hypothesis for this question, that there is no significant difference or relationship between microcystin concentrations throughout the years and months.
